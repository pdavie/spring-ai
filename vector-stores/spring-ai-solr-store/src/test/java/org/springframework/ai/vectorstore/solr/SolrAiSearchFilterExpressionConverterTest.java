...
class SolrAiSearchFilterExpressionConverterTest {

	final FilterExpressionConverter converter = new SolrAiSearchFilterExpressionConverter();

	@Test
	public void testDate() {
		String vectorExpr = this.converter.convertExpression(new Filter.Expression(ExpressionType.EQ,
	...
	public void tesEqAndGte() {
		String vectorExpr = this.converter.convertExpression(new Filter.Expression(ExpressionType.AND,
		...
		assertThat(vectorExpr).isEqualTo("metadata.genre:drama AND metadata.year:[2020 TO *]");
	}

	@Test
	public void tesIn() {
		String vectorExpr = this.converter.convertExpression(new Filter.Expression(ExpressionType.IN,
	...
	@Test
	public void testNe() {
		String vectorExpr = this.converter.convertExpression(new Filter.Expression(ExpressionType.OR,
				new Filter.Expression(ExpressionType.GTE, new Filter.Key("year"), new Filter.Value(2020)),
		...
		assertThat(vectorExpr)
			.isEqualTo("metadata.year:[2020 TO *] OR metadata.country:BG AND metadata.city: NOT Sofia");
	}
	...
	public void testGroup() {
		...
		assertThat(vectorExpr)
			.isEqualTo("(metadata.year:[2020 TO *] OR metadata.country:BG) AND NOT (metadata.city:Sofia OR Plovdiv)");
	}
	...
	public void tesBoolean() {
		...
		assertThat(vectorExpr)
			.isEqualTo("metadata.isOpen:true AND metadata.year:[2020 TO *] AND (metadata.country:BG OR NL OR US)");
	}
	...
	public void testDecimal() {
		...
		assertThat(vectorExpr).isEqualTo("metadata.temperature:[-15.6 TO *] AND metadata.temperature:[* TO 20.13]");
	}
